...
void opof_server(const char* address, unsigned short port, const char* cert, const char* key){
  ...
  std::cout << "Using CQ_TIMEOUT_MSEC value of " << cq_timeout_msec << std::endl;
  ...
  builder.SetSyncServerOption(ServerBuilder::SyncServerOption::CQ_TIMEOUT_MSEC, cq_timeout_msec);
  ...
  if (!server) {
    std::cerr << "Failed to start grpc server at " << cppaddress << std::endl;
    exit(1);
  }
